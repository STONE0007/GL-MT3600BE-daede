...
专为（MediaTek Filogic 820/880 平台）深度打造的电竞级优化 OpenWrt 固件。全面解锁内核级 Netkit 虚拟网卡通道与原生 eBPF 性能，集成 `dae` 与 `daed` 双完全体代理内核，并通过多层白名单机制剔除冗余运行时，实现仅约 **46MB** 的极致轻量体积与低抖动游戏转发。
